...
- Навигация на строках callback_data: `category:<id>` →
  ...
  возврат из карточки — те же `page:<key>:<стр>:<подкатегория>:<страна>` / `back:main`
  (id всех сообщений фотоальбома карточки хранятся в FSM-state пользователя под
  ключом `product_photo_msg_ids` и удаляются целиком при возврате — у альбома
  отдельное сообщение на каждое фото, в callback_data их не поместить из-за лимита
  64 байт), заявка `order:<id>` / `order:confirm|cancel`.
